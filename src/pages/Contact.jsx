...
const copyLink=(link)=>{
  ...
  return (
    <div className='py-4 px-2 px-sm-4 px-md-5'>
        <h2 className='fw-semibold my-4 text-center'>Contact Information</h2>
       ...
       {contact.map((item,index)=>{
            return <div key={index} className='mb-3  col-md-6 col-lg-4 px-0 px-sm-2  '  >
             ...
             <button onClick={()=>copyLink(item.link)} className='contactBtn text-white rounded-5 px-4 pb-2'>Copy</button>
             </div>
             </div>
            </div>
        })}
      ...
      <h5 className="fw-semibold my-4 text-warning mb-5 pb-5">1g17 နှင့်ပတ်သက်ပြီးအကြံပေးချက် များမေးမြန်း လိုသည်များ ရှိပါ က ကျွန်ုပ် Customer service. ယ် မေးမြန်းနိုင်ငံ မေးမြန်းနိုင်ပါ သည်။လူကြီးမင်း တို့၏မည်သည်အခက်အခဲကိုမဆိုဖြေရှ င်းပေးနိုင်ရန် အစဉ်ကြို ဆိုနေပါသည် 
       </h5>
     </div>
  )
}
