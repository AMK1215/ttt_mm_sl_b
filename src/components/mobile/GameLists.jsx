...
import { useLocation, useNavigate, useSearchParams } from 'react-router-dom'
 const GameLists = () => {
  ...
  return (<>
    {lists?.length>0 && <div className='gameProviders py-3 px-2 px-sm-4 gap-1 gap-sm-2 gap-lg-3 gap-lg-0 d-flex algin-items-center '>
      {lists?.map((list)=>{
        return <div onClick={()=>{
          navigate(window.location.pathname==='/games' ? `/games?type=${searchParams.get('type')}&list=${list.name}` : `/demo-play?type=${searchParams.get('type')}&list=${list.name}`)
